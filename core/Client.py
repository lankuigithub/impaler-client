...
class Socket(object):
    ...
    def receive(self):
        ...
        if len(self.__data) < self.__headerSize + data_length + len(self.__delimiter):
            return ''
        command_data = self.__data[self.__headerSize: self.__headerSize + data_length]
        self.__data = self.__data[self.__headerSize + data_length + len(self.__delimiter):len(self.__data)]
        command = Command()
    ...
    def send_command(self, command):
        ...
        self.__socket.send(self.__delimiter)
